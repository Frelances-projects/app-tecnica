      ...
      <TableComponent>
        ...
        <TableBody>
          {students.map(student => {
            const completedLessons = student.scheduledClass?.filter(scheduledClass => scheduledClass.status === 'COMPLETED' || scheduledClass.status === 'CONFIRMED')
            const completedCodeLessons = completedLessons?.filter(lesson => lesson.class.category === 'THEORETICAL').length || 0
            const completedPracticalLessons = completedLessons?.filter(lesson => lesson.class.category === 'PRACTICAL').length || 0
            ...
            return (
              <TableRow key={student.id}>
                ...
                <TableCell align="center">{completedPracticalLessons}</TableCell>
                {hasEditStudentModal && (
                  <TableCell>
                    <EditStudentModal
                      student={student}
