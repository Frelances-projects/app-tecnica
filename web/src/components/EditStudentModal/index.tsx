...
export function EditStudentModal({ student, categoryCard, schools }: EditStudentModalProps) {
      ...
      if (error instanceof AxiosError) {
        if (error.response?.data?.message) {
          if (error.response?.data.message === errorMessages.emailHasAlreadyBeenUsed) {
            return toast({
              title: 'Error!',
              description: 'Esse E-mail já está sendo utilizando, por favor coloque outro E-mail',
              variant: 'destructive'
            })
          } else if (error.response?.data.message === errorMessages.numberHasAlreadyBeenUsed) {
            return toast({
              title: 'Error!',
              description: 'Esse número já está sendo utilizado, por favor coloque outro',
              variant: 'destructive'
            })
          }
        } else {
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent className="w-full max-w-xl overflow-y-auto">
        <h1 className="font-bold text-lg">Editar aluno: {student.name}</h1>
